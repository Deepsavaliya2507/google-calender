...
import {getStorage, listEvents, sortPin, sortName, sortCourse, sortDate, sortCheck, determineTaskCourse, appendLastSort} from "./functions/DataFunctions"
...
export default class App extends React.Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      if (!this.state.loaded) {
        this.errorTimeoutOpen("Google API Timeout. Check console/config.");
      }
    }, 5000);
  }
  ...
  handleItemClick(event, name, calendarObjects) {
    if (name === 'signIn') {
  ...
  updateDone(id){
    for (let i = 0; i < this.state.calendarObjects.length; i++) {
      if(this.state.calendarObjects[i].id === id){
        if(this.state.calendarObjects[i].done===true){
          ...
          ApiCalendar.updateEvent(event, id).then(() => {
            let newCalendarObjects = [...this.state.calendarObjects];
            newCalendarObjects[i] = {...newCalendarObjects[i], done: false};
            this.setState({
              calendarObjects: newCalendarObjects
            })
          }
          ).catch((error) => {
            this.errorTimeoutOpen("Error 401/404")
          ...
          ApiCalendar.updateEvent(event, id).then(() => {
            let newCalendarObjects = [...this.state.calendarObjects];
            newCalendarObjects[i] = {...newCalendarObjects[i], done: true, pin: false};
            this.setState({
              calendarObjects: sortPin(newCalendarObjects)
            })
          }
          ).catch((error) => {
            this.errorTimeoutOpen("Error 401/404")
    ...
    for (let i = 0; i < this.state.calendarObjects.length; i++) {
      if(this.state.calendarObjects[i].id === id){
        if(this.state.calendarObjects[i].pin===true){
          ...
          ApiCalendar.updateEvent(event, id).then(() => {
            let newCalendarObjects = [...this.state.calendarObjects];
            newCalendarObjects[i] = {...newCalendarObjects[i], pin: false};
            this.setState({
              calendarObjects: sortPin(newCalendarObjects)
            })
          }
          ...
          ApiCalendar.updateEvent(event, id).then(() => {
            let newCalendarObjects = [...this.state.calendarObjects];
            newCalendarObjects[i] = {...newCalendarObjects[i], pin: true};
            this.setState({
              calendarObjects: sortPin(newCalendarObjects)
            })
          }
  ...
  async getEventObjects(calendarIDsPassed){
    ...
    for(let z = 0; z < calendarIDsPassed.length; z++){
      let calendarIDPassed = "";
      ...
      await listEvents(getSettingsValue("numEvents"),getSettingsValue("hoursBefore")).then(async ({result}) => {
        ...
        lastSortList.forEach(function(sortElement){
  ...
  render() {
    ...
    if(this.state.homePage){
      return(<HomePage/>)
    } else if(this.state.loaded===false && this.state.errorTimeoutOpen===false){
